...
<template>
  <div class="centermap">
    ...
    <div class="mapwrap">
      <dv-border-box-13>
        <div class="quanguo" @click="getchina('china')" v-if="code !== 'china'">
          中国
        </div>
...
export default {
  ...
  methods: {
	...
	getchina(code) {
				 this.scode = code;
				    eventBus.$emit('get-all-data'); // 发布获取全部数据的自定义事件
				    this.getData(code);
    ...
    mapclick() {
      ...
      this.$refs.CenterMap.chart.on("click", (params) => {
        ...
        if (xzqData) {
			eventBus.$emit('map-province-clicked', params);
